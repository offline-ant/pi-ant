...
export default function delegateExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    name: "delegate",
    label: "Delegate",
    description: "Run one task in an ephemeral Herdr worker and wait for its result and automatic retrospective. context='inherit' continues from the current conversation and excludes the delegate call itself; sibling tool results are not inherited. context='project' or 'clean' starts a blank conversation and is safe to run in parallel for independent tasks. context is required. Failures throw with recovery details.",
    ...
    promptGuidelines: [
      ...
      "Run delegate calls in parallel only when their tasks are independent and non-overlapping; sibling tool results are not visible inside inherited delegates.",
    ],
