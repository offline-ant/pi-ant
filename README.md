...
- Call-frame commands: `/bobs-mode [on|off|status|toggle]` toggles Bob's mode and can restrict the root tool set to `call` and `ask`; `/finish-call-now "message"` is a child-frame recovery command that overrides the call result with `message` and shuts down the worker. See `HOWTO-CALL-MODE.md`.
