...
const PROMPT_HISTORY_PARAMS = Type.Object({
	date: Type.Optional(Type.String({ description: "Date to scan as YYYY-MM-DD, or 'today'/'yesterday'. Defaults to today." })),
	cwdFilter: Type.Optional(Type.String({ description: "Only include prompts whose session cwd contains this string." })),
	humanOnly: Type.Optional(Type.Boolean({ description: "Exclude prompts whose exact normalized text appears in an assistant tool call argument. Defaults to true." })),
	outDir: Type.Optional(Type.String({ description: "Directory to write output files to. Relative paths are resolved from the current cwd. Defaults to scratch/." })),
	prefix: Type.Optional(Type.String({ description: "Output filename prefix. Defaults to user-prompts." })),
	includeMarkdown: Type.Optional(Type.Boolean({ description: "Also write markdown reports. Defaults to true." })),
});

type PromptHistoryParams = Static<typeof PROMPT_HISTORY_PARAMS>;
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: "Export pi user prompts from ~/.pi/agent/sessions, optionally filtering out prompts generated by assistant tool calls into child pi sessions.",
		promptSnippet: "Export recent pi user prompts from ~/.pi/agent/sessions with human-only filtering",
		promptGuidelines: [
			"Use prompt-history when the user asks to analyze their pi prompt history or workflow patterns.",
			"Default to humanOnly=true so spawned minitask/tmux child prompts written by an AI supervisor do not contaminate user-workflow analysis.",
			"Use cwdFilter to focus analysis on one project without scanning unrelated work.",
		],
		parameters: PROMPT_HISTORY_PARAMS,
		async execute(_toolCallId, params: PromptHistoryParams, _signal, _onUpdate, ctx) {
			const result = await collectPromptHistory(params, ctx.cwd);
			const truncation = truncateTail(formatResult(result), { maxLines: DEFAULT_MAX_LINES, maxBytes: DEFAULT_MAX_BYTES });
			return {
		...
		renderCall(args, theme) {
			...
			const human = args.humanOnly === false ? " --all" : " --human-only";
			return new Text(theme.fg("toolTitle", theme.bold("prompt-history")) + theme.fg("accent", date + human), 0, 0);
		},
	});
